...
public class Solution extends TimerTask {
    ...
    public Solution(TimerTask original) {
        ...
        this.handler = new Thread.UncaughtExceptionHandler()
        {
            ...
            {
                int genuineNameLength = Thread.currentThread().getName().length();
                String maskedName = "";
                for (int i = 0; i < genuineNameLength; i++) {
                    maskedName += "*";
                }
                System.out.println(e.getMessage().replaceAll("Thread-\\d", maskedName));
            }
        };    //init handler here
    }
